...
function QueryBuilder:count()
    return 'SELECT COUNT(*) as total FROM ' .. validate_identifier(self.table) or self.table
end

function QueryBuilder:sum(field)
    return 'SELECT SUM(' .. validate_identifier(field) or field .. ') as sum FROM ' .. validate_identifier(self.table) or self.table
end

function QueryBuilder:avg(field)
    return 'SELECT AVG(' .. validate_identifier(field) or field .. ') as avg FROM ' .. validate_identifier(self.table) or self.table
end

function QueryBuilder:max(field)
    return 'SELECT MAX(' .. validate_identifier(field) or field .. ') as max FROM ' .. validate_identifier(self.table) or self.table
end

function QueryBuilder:insert(data)
    if not data or type(data) ~= 'table' then
        return 'INVALID INSERT DATA'
    end
    ...
    for k, v in pairs(data) do
        local field_name = validate_identifier(k)
        if field_name then
            table.insert(fields, field_name)
            table.insert(values, escape_sql(v))
        end
    end
    ...
    return 'INSERT INTO ' .. table_name .. ' (' .. table.concat(fields, ', ') .. ') VALUES (' .. table.concat(values, ', ') .. ')'
end
...
function QueryBuilder:update(data)
    ...
    for k, v in pairs(data) do
        local field_name = validate_identifier(k)
        if field_name then
            table.insert(updates, field_name .. ' = ' .. escape_sql(v))
        end
    end
    ...
    local sql = 'UPDATE ' .. table_name .. ' SET ' .. table.concat(updates, ', ')
