...
> **本项目由 AI 开发** - 使用 [OpenCode](http://opencode.ai) 平台。所有代码（包括本 README.md）均由 AI 生成。
>
> **AI-Developed Project** - Built using [OpenCode](http://opencode.ai) platform. All code (including this README.md) was generated by AI.
...
本项目需要 LuaJIT 2.1 运行单元测试。默认路径为 `/usr/local/bin/luajit`。

This project requires LuaJIT 2.1 to run unit tests. Default path is `/usr/local/bin/luajit`.

```bash
# 验证 LuaJIT 安装（已就绪 /usr/local/bin/luajit）
luajit -v
...
**注意**：生产环境的实际配置位于服务器上的 `/usr/local/nginx/conf/nginx.conf`，该文件会 include 本项目中的 `myresty.conf`。

**Note**: The actual production configuration is located at `/usr/local/nginx/conf/nginx.conf` on the server, which includes the `myresty.conf` file from this project.
